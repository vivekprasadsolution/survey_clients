...
function SPLAddressChange(AddressCombo)
{
    ...
    document.getElementById("SPLSearchArea").innerHTML = '<select class="form-control" disabled="disabled"><option value=""></option></select>';
}
...
function SPLhandleServerResponse()
{
    ...
    else
    {
        ...
        else
        {
            ...
            //document.getElementById("house").value = LINE1;
            if (LINE3 !== '')
            {
                document.getElementById("address").value = LINE1 + ', ' + LINE2 + ', ' + LINE3;
            }
            else if (LINE2 !== '')
            {
                document.getElementById("address").value = LINE1 + ', ' + LINE2;
            }
            else
            {
                document.getElementById("address").value = LINE1;
            }
            ...
            if (COMP != "")
            {
                MultiLineAddress = MultiLineAddress + COMP + "\n"
            }
            if (LINE1 != "")
            {
                MultiLineAddress = MultiLineAddress + LINE1 + "\n"
            }
            if (LINE2 != "")
            {
                MultiLineAddress = MultiLineAddress + LINE2 + "\n"
            }
            if (LINE3 != "")
            {
                MultiLineAddress = MultiLineAddress + LINE3 + "\n"
            }
            if (TOWN != "")
            {
                MultiLineAddress = MultiLineAddress + TOWN + "\n"
            }
            if (COUNTY != "")
            {
                MultiLineAddress = MultiLineAddress + COUNTY + "\n"
            }
            if (POSTCODE != "")
            {
                MultiLineAddress = MultiLineAddress + POSTCODE + "\n"
            }
            if (COUNTRY != "")
            {
                MultiLineAddress = MultiLineAddress + COUNTRY + "\n"
            }
